...
public class MandoController {
    ...
    public static void processSMS(String s) {
        ...
        if (words[1].equals(commands.get(1)) && isActive.get(1)){
                if (words.length != 3) return; //invalid SMS
                    try {
        ...
        if (words[1].equals(commands.get(3)) && isActive.get(3)){
            if (words.length != 2) return; //invalid SMS
    ...
    public static String getHelp(ArrayList<String> commands, ArrayList<Boolean> isActive) {
            String msg = "help: ";
            if (isActive.get(0)) //forward sms
                msg += "\nForward SMS:\n<PIN> " + commands.get(0) + " <No.Tujuan> <SMS>\n";
            if (isActive.get(1)) //ambil sms
                msg += "\nretieve SMS:\n<PIN> " + commands.get(1) + " <Jumlah SMS yang akan diambil>\n";
            if (isActive.get(2)) //kontak
                msg += "\nget contact:\n<PIN> " + commands.get(2) + " <Nama kontak>\n";
            if (isActive.get(3)) //help
                msg += "\nhelp:\n<PIN> " + commands.get(3) + "\n";
            if (isActive.get(4)) //suara
                msg += "\nrecord sound:\n<PIN> " + commands.get(4) + " <waktu rekam(detik)>\n";
    ...
    private static boolean match(String a, String b) {
            ...
            for (int i=0;i<a.length() && j + 1< b.length();i++)
                if (a.charAt(i) == b.charAt(j)) j++; else j = 0;
            return j + 1 == b.length();
        }
    ...
    public static String getContacts(String x) {
            ...
            if (msg.length() == 0) return "Kontak tidak ditemukan";
            return msg;
        }
